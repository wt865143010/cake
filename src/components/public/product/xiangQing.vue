<template>
    <div id="main">
      <div id="myMain">
        <div class="img-top"><img src="../../assets/allProductImg/ai.png" alt=""></div>
        <div id="div-main">
          <div class="div-l">
            <div class="div-l1">
              <h2>{{myxq.name}}<span>商品评价</span></h2>
              <div class="left-t">
                <p class="p-1">{{myxq.english_name}}</p>
                <p id="p-2">{{myxq.time}}</p>
                <p class="p-3">
...
<!--                  <span>水果</span>-->
              ...
              <div class="left-b">
                ...
                <p class="p-4"><span>甜味</span><span>
                  <span>{{myxq.sweet}}级</span>

                </span></p>
                <p class="p-5">{{myxq.free}}</p>
                <p class="p-6">{{mysize}}</p>
                <p class="p-7">若不及时食用，请放置0-10℃冷藏</p>
              </div>
            </div>
            <div class="div-b">
<!--              <div class="kong"></div>-->
              <div class="div-w" v-for="(value,index) in xq" @click="getOther($event,index)">
                <p class="p-8">{{value.specs_value}}</p>
              </div>
            </div>
          </div>
          <div class="div-r">
            ...
            <div class="r-bottom">
              <p class="p-9"><span class="s-9-1">￥</span><span class="s-9-2">{{jiage}}</span></p>
              <p class="p-10">
                <button class="btn1" @click="addCar">加入购物车</button>
                <button class="btn2">立即订购</button>
              </p>
            </div>
          </div>
        </div>
        <div class="img-b"><img src="../../assets/allProductImg/ai1.png" alt=""></div>
      </div>
    </div>
</template>
...
<script>
    export default {
      ...
      created() {
          let that=this;
          this.$axios.get("xiangQing.do?id=17").then(function (resp) {
            that.xq=resp.data;
            console.log(that.xq);
          });
          this.$axios.get("xiangQing1.do?id=17").then(function (resp) {
            that.myxq=resp.data[0];
            console.log(that.myxq);
          });
          this.$axios.get("xiangQing2.do",{params:{id:17}}).then(function (resp) {
      ...
      methods:{
          getOther(e,index){
            ...
            for (let i=0;i<mynode.length;i++){
              if (i==index){
                e.target.setAttribute("class","p-8 p-8-1");
                 that.productspecs= e.target.textContent;
                console.log(that.productspecs)
              }else {
                mynode[i].childNodes[0].setAttribute("class","p-8")
              }
            }
        ...
        addCar(){
            let that=this;
            let addxq=that.myxq;
            console.log([addxq.name,addxq.english_name,addxq.imgsrc,that.productspecs,1,1,that.jiage]);
            // this.$axios.get("xqCar.do",{params:{}})
        }
